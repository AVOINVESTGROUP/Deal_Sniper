...
Статус исполнения: `R8.1-PROD` завершён 29 июля 2026 года. Immutable API и publisher
активны в production, bounded delivery и повторный idempotency run успешны. `R8.1-PILOT`
остаётся этапом наблюдения за следующими подходящими сделками.

Владелец утвердил R8 и потребовал завершить работающий production-проект без отдельного
тестового Telegram-канала. Firebase и Admin Auth в R8.1 не изменялись.
...
Первоначальное требование отдельного тестового Telegram Pro-канала отменено владельцем.
Delivery-off staging подтвердил изоляцию и идемпотентность, а ограниченный реальный smoke
выполнен непосредственно в существующем production Pro-канале.
