...
Для настоящего Telegram smoke нужен отдельный тестовый канал, отличный от production Free/Pro.
Бот должен быть его администратором. После получения тестового recipient delivery будет временно
включена с лимитом `1 deal + 1 digest`, затем снова выключена и очередь остановлена.

До успешного smoke команда `Разрешаю production deploy R8.1` не должна исполняться.
