...
Владелец сообщил, что платных пользователей пока нет, и отклонил отдельный тестовый канал. Read-only аудит выявил отдельный release blocker: периодический publisher не восстанавливает пропущенные `pro/v1` события. В production есть 3 текущих `INSPECT`, но только 5 исторических Pro-доставок. Исправление и controlled smoke описаны в `docs/PLAN_R7_1_PRO_PUBLICATION.md`; до утверждения этого плана код не менять.
