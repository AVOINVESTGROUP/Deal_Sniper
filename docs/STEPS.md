...
- [x] Перед продуктовым релизом `0.12` в план добавлены обязательные стабилизационные релизы `0.11A–0.11D`; код по замечаниям ещё не изменялся.
...
- [x] Прежние pilot/backfill результаты помечены диагностическими; официальный пилот начинается заново после migration reconciliation `0.11M` и baseline `0.11D`.
