...
Браузерный Admin Web обращается к API только по same-origin путям Firebase Hosting (`/admin/**`, `/content/**`, `/version`, `/health`), которые переписываются на Cloud Run. Это исключает зависимость интерфейса от CORS и доступности домена API Gateway. Cloud Run допускает сетевой вызов от Hosting, но каждый `/admin/**` запрос по-прежнему требует действительный Firebase ID token и email из `ADMIN_EMAILS`.
