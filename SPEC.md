...
Перед подключением каждого внешнего источника проверяются условия использования, наличие официального API или разрешённого способа доступа, авторизация, лимиты и стабильность схемы.
...
Получают объявления с фиксированной ценой. Рекомендуемая базовая частота — раз в 5–15 минут с учётом правил площадки. Коллектор сохраняет исходные данные до нормализации и не вызывает Gemini.
...
- **Firebase Hosting** — статический frontend будущей TMA и проксирование `/api/**` в Cloud Run;
- **Firebase Authentication** — пользовательская сессия TMA после серверной проверки Telegram `initData`;
...
1. Google Cloud foundation, IAM, Secret Manager и Infrastructure as Code.
2. Firestore-модель, индексы, Security Rules и идемпотентность.
3. Реестр источников и первый Cloud Run Job без неявного mock fallback.
4. Cloud Storage для сырых снимков, Cloud Scheduler и Cloud Tasks.
5. Нормализация и межсайтовое объединение автомобилей.
6. Движок сопоставимых цен.
7. Модель расходов, рисков и детерминированный Decision Engine.
8. Vertex AI enrichment через `google-genai`.
9. Cloud Run Application API и Telegram webhook.
10. Простой Telegram-бот и повтор доставки через Cloud Tasks.
11. Пилот на реальных объявлениях и контроль облачных расходов.
12. Подключение дополнительных классифайдов отдельными Cloud Run Jobs.
13. Telegram Mini App на Firebase Hosting/Auth поверх стабильного API.
14. Сохранение фактических результатов сделок и опциональная аналитика BigQuery.
