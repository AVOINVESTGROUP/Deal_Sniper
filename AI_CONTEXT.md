...
27 июля 2026 владелец остановил работу из-за нарушения обязательного процесса анализа. Выполнен сквозной read-only аудит SPEC, архитектуры, operations, Admin browser path, publication identity, Free/Pro и кандидата `1ce36ff`. Production не менялся и остаётся на commit `12bdee56c6b299132f55d1afedc0d25e4918ac82`, digest `sha256:561814a852339e454dca7a362d41bc68e27ffe3359fc02e5eedbe6a31597aa3e`. Кандидат `1ce36ff` запрещён к deploy: retry/allSettled не доказывают устранение Gateway/CORS причины; `format_market_watch_card` раскрывает в Free цену, verified market и listing URL вопреки SPEC; immutable `PublicationEvent` с прежним ID игнорирует CTA-поля через `create`/`INSERT OR IGNORE`; Firebase rewrites противоречат принятому Gateway-only контуру; отсутствуют browser integration, staging rehearsal и immutable release evidence. Полный план R6 находится в `docs/PLAN_REVIEW_2026-07-27-R6.md`. До явного утверждения владельцем разрешены только документы и read-only проверки.
