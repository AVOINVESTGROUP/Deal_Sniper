...
После входа Chrome владельца показал `Failed to fetch`. Полная серверная браузерная матрица с `Origin=https://avo-deal-sniper.web.app` подтвердила HTTP 200 и CORS для overview, market pulse, preview и обоих outbox запросов, поэтому критический браузерный путь заменяется на same-origin Firebase Hosting rewrites. Cloud Run ingress разрешается для Hosting, но application-level Firebase authorization остаётся обязательной для `/admin/**`. Middleware сохраняет прямой bearer от Hosting и восстанавливает forwarded bearer только для API Gateway.
