...
27 июля 2026 после утверждения владельцем реализован CTA Free → Pro. Добавлена библиотека из 30 проверяемых англоязычных вариантов; атомарный allocator SQLite/Firestore закрепляет вариант за `publication_id`, а `PublicationEvent` и outbox сохраняют текст, label, target, fingerprint и template version. Retry использует тот же вариант. Автомобильная Free-публикация fail-closed без корректной HTTPS Telegram-ссылки `TELEGRAM_PRO_SUBSCRIPTION_URL`. Одновременно Admin Web стал устойчив к временным Gateway/CORS-сбоям: retry, однократное обновление Firebase token после `401` и частичный рендер через `Promise.allSettled`. CSP Hosting разрешает необходимые Firebase runtime-модули. Gate: Ruff, mypy, 71 pytest. Production ещё работает на старом digest; gcloud требует повторной интерактивной авторизации, поэтому deploy и live smoke не подтверждены.
