...
- Проверка draft PR #1 подтвердила, что `main` остаётся legacy и отстаёт на 17 коммитов, а развёрнутый production собран из неслитой рабочей ветки.
- До Free/Pro, поиска и панели обязательны `0.11A–0.11D`: точный snapshot provenance, изоляция пользователей и роли, корректный retry/outbox, подтверждённые аналоги и экономика, lifecycle/масштабирование/IaC, затем воспроизводимое слияние в `main` и deployment по image digest.
