...
28 июля 2026 R7 дошёл до staging: evidence head `02fcb6f`, Cloud Build `1a47a7e3-cf4c-4613-ac3f-543a3ee3c0b6`, immutable digest `sha256:ab0b8880041985c47bf2a7eb69b638ed6d2370a21e3e5044b42ebfe4e2ffe94a`, Cloud Run staging revision `deal-sniper-api-staging-00026-cfj`, Gateway config `r7-02fcb6f`. Firestore runtime-config integration и authenticated Chrome/CORS smoke всех Admin read paths прошли при `DELIVERY_ENABLED=false`; временный Firebase user удалён и allowlist восстановлен. Остались Hosting Preview после `firebase login --reauth` и мутационный Stars/rollback smoke в отдельном тестовом Pro-канале. Production не менялся и остаётся на R6 `851ddaf` / `sha256:c2e55a…a9112`; отдельного разрешения на production deploy R7 нет.
