...
28 июля 2026 R7 дошёл до staging: evidence head `02fcb6f`, Cloud Build `1a47a7e3-cf4c-4613-ac3f-543a3ee3c0b6`, прежний digest `sha256:ab0b8880041985c47bf2a7eb69b638ed6d2370a21e3e5044b42ebfe4e2ffe94a`, Cloud Run staging revision `deal-sniper-api-staging-00030-ps7`, Gateway config `r7-02fcb6f`. Firestore runtime-config integration и authenticated Chrome/API smoke прошли при `DELIVERY_ENABLED=false`; временные Firebase users удалены и allowlist восстановлен. После Firebase re-auth Hosting Preview создан, но настоящий Preview UI выявил HTTP 400 CORS: код жёстко разрешал лишь production origins. Реализован явный `CORS_ALLOWED_ORIGINS` без wildcard с fail-fast validation; локальный gate нового кандидата дал Ruff, strict mypy, 90 passed / 2 skipped, coverage 64%, dependency audit и Terraform validate. Прежний digest аннулирован, нужен новый immutable RC и повтор staging. Отдельно остаётся мутационный Stars/rollback smoke в тестовом Pro-канале. Production не менялся и остаётся на R6 `851ddaf` / `sha256:c2e55a…a9112`; отдельного разрешения на production deploy R7 нет.
